/*
  ...
  1. Database Function Fixes
    - Fix ambiguous column references in log_user_activity
    - Fix session token handling in start_user_session
    - Improve error handling in all RPC functions
    
  2. RLS Policy Updates
    - Fix users table UPDATE policy for proper data modifications
...
DO $$ 
BEGIN
    -- Drop log_user_activity function variants
    ...
    -- Drop start_user_session function variants
    ...
    -- Drop end_user_session function variants
...
BEGIN
  ...
  -- Insert activity log with properly qualified column names
...
BEGIN
  ...
  -- Create new session with proper token handling
...
-- Create updated RLS policy for users table
...
-- Also grant to anon for certain functions that might be called during signup
GRANT EXECUTE ON FUNCTION log_user_activity(text, text, jsonb, text, integer, integer, text) TO anon;