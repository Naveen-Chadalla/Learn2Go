/*
  # Session Isolation and Activity Tracking Improvements

  1. Security
    - Improved RLS policies for user_activity_logs
    - Added session isolation to prevent multiple active sessions per user
    - Created functions for secure activity tracking

  2. Performance
    - Added indexes for faster session lookups
    - Implemented cleanup functions for old data

  3. Features
    - Enhanced activity logging with session context
    - Added user session management functions
*/
...
CREATE POLICY "Users can view own activity logs"
  ...
  USING (
    username = COALESCE(
      ((current_setting('request.jwt.claims', true))::json -> 'user_metadata'::text) ->> 'username'::text,
      (current_setting('request.jwt.claims', true))::json ->> 'email'::text,
      split_part((current_setting('request.jwt.claims', true))::json ->> 'email'::text, '@'::text, 1)
    )
    OR ((current_setting('request.jwt.claims', true))::json -> 'user_metadata'::text) ->> 'username'::text = 'Hari'::text
  );
...
CREATE POLICY "Admin can manage all activity logs"
  ...
  FOR ALL
  TO authenticated
  USING (((current_setting('request.jwt.claims', true))::json -> 'user_metadata'::text) ->> 'username'::text = 'Hari'::text)
  WITH CHECK (((current_setting('request.jwt.claims', true))::json -> 'user_metadata'::text) ->> 'username'::text = 'Hari'::text);
