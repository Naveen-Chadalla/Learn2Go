/*
  # Fix function conflicts and RLS policies

  1. Database Functions
    - Drop all existing conflicting functions with specific signatures
    - Create new session management functions
    - Create new activity logging functions
...
-- First, get a list of all existing function signatures and drop them specifically
DO $$
DECLARE
    func_record RECORD;
BEGIN
    ...
    FOR func_record IN 
        ...
        AND p.proname IN ('log_user_activity', 'start_user_session', 'end_user_session')
    LOOP
...
-- Create the session management function with a unique name
CREATE FUNCTION create_user_session_v2(
  user_name text,
  session_token text,
...
-- Create the session termination function with a unique name
CREATE FUNCTION terminate_user_session_v2(
  user_name text,
  token_value text
) RETURNS void AS $$
DECLARE
...
-- Create the activity logging function with a unique name
CREATE FUNCTION record_user_activity_v2(
  user_name text,
  activity text,
...
-- Now create the wrapper functions with the original names
...
BEGIN
  PERFORM create_user_session_v2(user_name, session_token, user_ip, user_agent_string);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  PERFORM terminate_user_session_v2(user_name, token_value);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  PERFORM record_user_activity_v2(user_name, activity, details, token_value, duration_secs, activity_score, page_path);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
GRANT EXECUTE ON FUNCTION create_user_session_v2 TO authenticated;
GRANT EXECUTE ON FUNCTION terminate_user_session_v2 TO authenticated;
GRANT EXECUTE ON FUNCTION record_user_activity_v2 TO authenticated;
