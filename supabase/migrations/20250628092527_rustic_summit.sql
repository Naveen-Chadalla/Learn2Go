/*
  # Session Isolation and Activity Logging Fix

  1. Security Updates
    - Fix RLS policies for user_activity_logs to allow proper session isolation
    - Ensure users can only access their own data
    - Allow system to log activities for authenticated users

  2. Session Management
    - Add session isolation constraints
    - Improve user session tracking
    - Add session cleanup functions

  3. Activity Logging
    - Fix activity logging permissions
    - Ensure proper user isolation in activity logs
...
CREATE POLICY "Users can view own activity logs"
  ...
  USING (
    username = COALESCE(
      ((jwt() -> 'user_metadata'::text) ->> 'username'::text),
      (jwt() ->> 'email'::text),
      split_part((jwt() ->> 'email'::text), '@'::text, 1)
    )
    OR ((jwt() -> 'user_metadata'::text) ->> 'username'::text) = 'Hari'::text
  );
...
CREATE POLICY "Admin can manage all activity logs"
  ...
  FOR ALL
  TO authenticated
  USING (((jwt() -> 'user_metadata'::text) ->> 'username'::text) = 'Hari'::text)
  WITH CHECK (((jwt() -> 'user_metadata'::text) ->> 'username'::text) = 'Hari'::text);
