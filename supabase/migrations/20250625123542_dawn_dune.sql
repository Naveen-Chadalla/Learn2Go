/*
  ...
  1. New Tables
    ...
    - Enhanced `users` table with activity fields
    - Enhanced `user_progress` table with detailed tracking

  2. Activity Tracking Features
    - Login/logout times with session duration
    - Lesson progress and quiz results
    - Game participation and scores
    - Page views and navigation tracking
    - Streak calculations and engagement metrics

  3. Admin Dashboard Features
    - Real-time user monitoring
    - Complete activity logs and audit trails
    - User account management with deletion capabilities
    - Session tracking and online status
...
-- Create view for admin dashboard with comprehensive user data
...
SELECT 
  ...
  u.average_quiz_score,
  u.best_quiz_score,
  u.current_page,
  u.last_lesson_completed,
  
  -- Session information
  s.session_token,
  s.login_time AS current_session_start,
  s.is_active AS currently_online,
  s.pages_visited AS current_session_pages,
  s.lessons_completed AS current_session_lessons,
  s.quizzes_taken AS current_session_quizzes,
  s.games_played AS current_session_games,
  
  -- Activity summary
  CASE 
    WHEN u.last_active > now() - INTERVAL '5 minutes' THEN 'online'
    WHEN u.last_active > now() - INTERVAL '1 hour' THEN 'recently_active'
    WHEN u.last_active > now() - INTERVAL '24 hours' THEN 'active_today'
    ELSE 'inactive'
  ...
  (SELECT COUNT(*) FROM user_activity_logs 
   ...
   AND timestamp > now() - INTERVAL '24 hours') AS activities_last_24h,
   
  -- Total activities
  (SELECT COUNT(*) FROM user_activity_logs 
   WHERE username = u.username) AS total_activities

FROM users u
LEFT JOIN user_sessions s ON u.username = s.username AND s.is_active = true
ORDER BY u.last_active DESC;

-- Grant permissions for the admin view
GRANT SELECT ON admin_user_dashboard TO authenticated;

-- Create RLS policy for admin view
CREATE POLICY "Admin can view dashboard"
  ON admin_user_dashboard
  FOR SELECT
  TO authenticated
  USING (((current_setting('request.jwt.claims'::text, true))::json ->> 'username'::text) = 'Hari');